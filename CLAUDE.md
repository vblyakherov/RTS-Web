...
- С 2026-06-05 исправлен XLSM sync:
  - `sync_template.xlsm` обновлён вручную и теперь содержит актуальный VBA;
  - `SyncNow()` сначала использует встроенный `_Config.auth_token`, а не вызывает `DoLogin()`;
  - `SERVER_URL` внутри VBA: `https://tracker.rtk-service.ru`;
  - новый Excel после download больше не спрашивает логин/пароль при первом sync;
  - последний prod-коммит: `3fe4957`.
...
- текущий инвентарь в коде:
  ...
  - frontend unit: `56`
  - e2e spec files: `7`
- после изменений 2026-06-05 полный regression ещё не прогонялся; prod XLSM sync подтверждён вручную после нового скачивания файла
